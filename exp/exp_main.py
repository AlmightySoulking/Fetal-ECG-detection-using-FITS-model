...
import numpy as np
# The standard experiment runner in the repo.
"""Primary workhouse for most of the models in the projects (like informer, Autonformer, DLinear, FEDformer.py, Film.py, etc.).

   The args object contains dozens of settings such as: 
   
   model --> The name of the model to use,
   data --> The name of the dataset to use,
   (seq_len, pred_len, label_len) --> Sequence length parameters,
   (learning_rate, batch_size, train_epoches) --> hyperparameters,
   use_gpu, gpu
...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        model = model_dict[self.args.model].Model(self.args).float() # Maps the model name from args.model to the actual model class
    ...
    def _get_data(self, flag):
        data_set, data_loader = data_provider(self.args, flag) #Task 1: understand the data_provider function in the data_provider.data_factory class.
    ...
    def _select_criterion(self):
        criterion = nn.CrossEntropyLoss() # Modified the Loss function to adjust to our classification Task.
        return criterion
